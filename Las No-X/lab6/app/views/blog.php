...
<main class="container">
    ...
    <section>
        <div id="content">
            <a href="/blog/create" class="btn btn-primary pull-right btn-my">Добавть запись</a>
            <p class="lead">Блог</p>
            ...
            <nav aria-label="Навигация по страницам">
                ...
                <div class="total-pages">Всего страниц: <strong><?= $paginate['total'] ?></strong></div>
                <div class="pagination">
                    <div <? if ( $paginate['is_first'] ): ?> class="disabled" <? endif ?><a href="/blog/page/<?= $paginate['prev'] ?>"
                                                                              aria-label="Предыдущая"><div class="navigationp"
                                aria-hidden="true">&laquo;</div></a></div>
                    <? for($i=$paginate['left_tail'];$i<$paginate['current'];$i++) : ?>
                    <div><a href="/blog/page/<?= $i ?>"><?= $i ?></a></div>
                    <? endfor ?>
                    <div class="active navigationp"><a href="/blog/page/<?= $paginate['current'] ?>"><?= $paginate['current'] ?><span
                                    class="sr-only">(текущая)</span></a></div>
                    <? for($i=$paginate['current']+1;$i<=$paginate['right_tail'];$i++) : ?>
                    <div><a href="/blog/page/<?= $i ?>"><?= $i ?></a></div>
                    <? endfor ?>
                    <div  <? if ($paginate['is_last']) : ?> class="disabled" <? endif ?>><a href="/blog/page/<?= $paginate['next'] ?>"
                                                                             aria-label="Следующая"><div class="navigationp"
                                aria-hidden="true">&raquo;</div></a></div>
                </div>
                <div class="clearfix"></div>
                </div>
            </nav>
